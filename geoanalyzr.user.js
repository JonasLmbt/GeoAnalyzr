...
<html>
  ...
  function normalizeLatLng(lat, lng) {
    ...
    if (Math.abs(lat) > 90 || Math.abs(lng) > 180) {
      return { lat: void 0, lng: void 0 };
    }
    return { lat, lng };
  }
  function computeBboxPolygon(coords) {
  ...
  function pointInMultiPolygon(lng, lat, mpoly) {
  ...
  function parseGeoJsonMaybe(data) {
  ...
  function getIso2FromProperties(props) {
    const candidates = [
      props?.ISO_A2,
      props?.iso_a2,
      props?.ISO2,
    ...
    countriesPromise = (async () => {
      ...
      for (const f of features) {
        ...
        if (type === "Polygon" && Array.isArray(coordinates)) {
          const bbox = computeBboxPolygon(coordinates);
          out.push({ iso2, geometryType: "Polygon", coordinates, bbox });
  ...
  async function resolveCountryCodeByLatLng(lat, lng) {
    ...
    try {
      ...
      for (const c of countries) {
        ...
        const hit = c.geometryType === "Polygon" ? pointInPolygon(norm.lng, norm.lat, c.coordinates) : pointInMultiPolygon(norm.lng, norm.lat, c.coordinates);
        if (hit) {
    ...
    for (let i = 0; i < rounds.length; i++) {
      ...
      if (family === "teamduels") {
        ...
        round.timeMs = (() => {
          const s = toTs(r?.startTime);
          const e = toTs(r?.endTime);
          return s !== void 0 && e !== void 0 && e >= s ? e - s : void 0;
        })();
        normalizedRounds.push(round);
      }
    }
  ...
  async function getAnalysisWindowData(filter) {
    ...
    for (const d of details) {
      ...
      for (const x of ids) {
        if (!x.id) continue;
        const cur2 = opponentCounts.get(x.id) || { games: 0 };
        cur2.games += 1;
        if (x.name) cur2.name = x.name;
        if (x.country) cur2.country = x.country;
