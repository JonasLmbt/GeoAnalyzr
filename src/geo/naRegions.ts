...
let usIndexPromise: Promise<FeatureIndexItem[]> | null = null;
let caIndexPromise: Promise<FeatureIndexItem[]> | null = null;

async function loadUsIndex(): Promise<FeatureIndexItem[]> {
  if (!usIndexPromise) {
    usIndexPromise = (async () => {
      const geo = await fetchJson(US_STATES_GEOJSON_URL);
...
async function loadCaIndex(): Promise<FeatureIndexItem[]> {
  if (!caIndexPromise) {
    caIndexPromise = (async () => {
      const geo = await fetchJson(CA_PROVINCES_GEOJSON_URL);
      const feats = Array.isArray(geo?.features) ? geo.features : [];
      const out: FeatureIndexItem[] = [];
      for (const f of feats) {
        const name = typeof f?.properties?.name === "string" ? f.properties.name.trim() : "";
...
const usMemo = new Map<string, string>();
const caMemo = new Map<string, string>();

export async function resolveUsStateByLatLng(lat: number, lng: number): Promise<string | null> {
...
export async function resolveCaProvinceByLatLng(lat: number, lng: number): Promise<string | null> {
  if (!Number.isFinite(lat) || !Number.isFinite(lng)) return null;
  const key = memoKey(lat, lng);
