...
export type ServerSyncStatus = {
  ...
  bytesJson: number;
  bytesGzip: number;
  chunks?: number; // present when a large sync was split into multiple uploads
};
...
async function buildDelta(
  ...
  opts: {
    compact: boolean;
    includeAggregates: boolean;
    maxCursorTo?: number; // optional upper bound for chunked sync
    forceFull?: boolean; // metadata flag (server may treat as a full rebuild)
    filterModeFamily: "all" | "duels" | "teamduels";
  ...
  const cursorUpper =
    typeof opts.maxCursorTo === "number" && Number.isFinite(opts.maxCursorTo) && opts.maxCursorTo > 0
      ? Math.max(cursorFrom, Math.floor(opts.maxCursorTo))
      : 0;

  const [ownerId, ownerName] = await Promise.all([getCurrentPlayerId(), getCurrentPlayerName()]);

  const byTime = <T>(table: any, index: string, tsFrom: number, tsTo: number, includeTo: boolean) => {
    const w = table.where(index);
    if (tsTo > 0) return w.between(tsFrom, tsTo, false, includeTo).toArray() as Promise<T[]>;
    return w.above(tsFrom).toArray() as Promise<T[]>;
  };

  const [gamesByTime, roundsByTime, detailsByTime, gameAggByTime] = await Promise.all([
    byTime<FeedGameRow>(db.games, "playedAt", cursorFrom, cursorUpper, true),
    byTime<RoundRow>(db.rounds, "playedAt", cursorFrom, cursorUpper, true),
    byTime<GameRow>(db.details, "fetchedAt", cursorFrom, cursorUpper, true),
    opts.includeAggregates ? byTime<GameAggRow>(db.gameAgg, "computedAt", cursorFrom, cursorUpper, true) : Promise.resolve([] as GameAggRow[])
  ]);
...
export async function runServerSyncOnceWithOptions(
  ...
  const MAX_GZIP_BYTES = 4.5 * 1024 * 1024; // keep below common reverse-proxy limits
  const MAX_CHUNKS = 60;
  ...
  const postDelta = async (d: { cursorFrom: number; cursorTo: number; counts: any; bytesJson: number; bytesGzip: Uint8Array }) => {
    const headers = buildHeaders(d);
    const res = await gmPostBytes(endpointUrl, d.bytesGzip, { headers, timeoutMs: 60000 });
  ...
  const pickChunkUpperBound = async (since: number, targetCount: number): Promise<number | null> => {
    ...
    const [games, rounds, details, aggs] = await Promise.all([
      ...
      settings.includeAggregates ? db.gameAgg.where("computedAt").above(since).limit(lim).toArray() : Promise.resolve([] as GameAggRow[])
    ]);

    const ts: number[] = [];
    for (const g of games as any[]) if (typeof g?.playedAt === "number" && Number.isFinite(g.playedAt) && g.playedAt > since) ts.push(g.playedAt);
    for (const r of rounds as any[]) if (typeof r?.playedAt === "number" && Number.isFinite(r.playedAt) && r.playedAt > since) ts.push(r.playedAt);
    for (const d of details as any[]) if (typeof d?.fetchedAt === "number" && Number.isFinite(d.fetchedAt) && d.fetchedAt > since) ts.push(d.fetchedAt);
    for (const a of aggs as any[]) if (typeof a?.computedAt === "number" && Number.isFinite(a.computedAt) && a.computedAt > since) ts.push(a.computedAt);
    ...
    const pick = uniq[idx] ?? uniq[uniq.length - 1];
    return typeof pick === "number" && Number.isFinite(pick) && pick > since ? pick : uniq[uniq.length - 1] ?? null;
  };

  const runChunked = async (startCursorFrom: number): Promise<ServerSyncStatus> => {
    ...
    let last: ServerSyncStatus | null = null;
    ...
    for (let guard = 0; guard < MAX_CHUNKS; guard++) {
      ...
      // No progress -> stop (avoid loops).
      if (!(delta.cursorTo > since)) break;

      // If still too large, shrink the window and retry.
      ...
      last = st;
      if (!st.ok) {
        // If the server still says "too large", shrink and retry same cursor.
      ...
      // If this chunk was comfortably small, we can try a slightly larger window next time.
      if (delta.bytesGzip.length < MAX_GZIP_BYTES * 0.55) targetCount = Math.min(20_000, Math.floor(targetCount * 1.35));
    }

    // If we never posted anything, fall back to a single delta (small/no-op).
    if (!last) {
  ...
  // Fast path: try a single delta first (except forceFull, which is likely large).
  if (!forceFull) {
